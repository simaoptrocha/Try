...
export const tokens: Record<string, { id: string; name: string; logo: string }[]> = {
  ...
  celo: [
    { id: "ceur", name: "cEUR", logo: "/images/tokens/ceur.svg" },
    { id: "cusd", name: "cUSD", logo: "/images/tokens/cusd.svg" }
  ]
};
...
export const chains = [
  ...
  },
  {
    ...
    token: 'ETH',
    label: 'Base',
    rpcUrl: 'https://base-rpc.publicnode.com', // Public RPC Gateway
    icon: 'base_logo',
    env: 'mainnet',
    name: 'base',
    defaultToken: 'usdc',
...
// Logic for environemnt handling
...
export const checkUserNextStep = async (
  router: NextRouter,
  ...
  if (status.status === 200) {
    ...
    if (status.data.status !== 'FULL_USER' && transactions?.data) {
      ...
      if (amount >= 700 || total >= 2) {
        await router.push('/kyc/intro');
        return;
      }
    }
    if (status.data?.status !== 'FULL_USER') {
      await router.push('/kyc/warning');
      return;
    }
...
export const getChainLabelByName = (chainName: string | undefined): string | undefined => {
